...
CREATE INDEX IF NOT EXISTS idx_doses_upcoming ON api.doses(scheduled_time, status)
  WHERE status = 'pending' AND scheduled_time BETWEEN NOW() AND NOW() + INTERVAL '2 hours';
...
- Refresh stats: */15 * * * * (every 15 minutes)
- Update statistics: 0 2 * * 0 (weekly)
- Performance monitoring: 0 */6 * * * (every 6 hours)
*/ 